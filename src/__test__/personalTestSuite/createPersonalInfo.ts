...
export const createPersonTests = () => describe('TEST SUITE 1 => create personal info route (POST) ', () => {
	describe('given that creation of person information is successful', () => {
		it('should return 200 with created personal data', async () => {
			const { statusCode, body } = await supertest(app).post('/personal').send(newPerson);
	...
	describe('given that creation failed due to invalid type of Employee address (only strnig allowed)', () => {
		it('should return 422 with error message', async () => {
			const { statusCode, body } = await supertest(app).post('/personal').send({
				'address': 234123,
				'phoneNo': '9765040510',
				'emailId': 'mahesh@gmail.com',
				'age': 22,
